...
class SDCarto(GeoAxes):
    ...
    def to_aagcms(self, lats, lons):
        ...
        if "aacgmv2" in self.coords: 
            for i in range(lats.shape[0]):
                mlats[i,:], mlons[i,:], mlt = aacgmv2.get_aacgm_coord_arr(lats[i,:], 
                                                                          lons[i,:], 
                                                                          300, self.plot_date)
    ...
    def overlay_radar(self, rad, tx=cartopy.crs.PlateCarree(), marker="D", zorder=2, 
                      markerColor="darkblue", markerSize=5, 
                      fontSize="small", font_color="k", xOffset=None, yOffset=-1.5, annotate=True):
        ...
        self.scatter([lon], [lat], s=markerSize, marker=marker,
                color=markerColor, zorder=zorder, transform=tx, lw=0.8)
    ...
    def overlay_fov(self, rad=None, tx=cartopy.crs.PlateCarree(), maxGate=100, rangeLimits=None, beamLimits=None,
                    ...
                    fovObj=None, zorder=2, lineColor="k", lineWidth=0.4, ls="-"):
        """ Overlay radar FoV """
        if hasattr(self, rad) and rad is None: rad = self.rad 
        ...
        xyz = self.projection.transform_points(tx, lonFull, latFull, 300*np.ones_like(latFull))
...
class FanPlots(object):
    ...
    def __init__(self, figsize=(4,4), nrows=1, ncols=1, dpi=300, coords="aacgmv2_mlt"):
        print("Fan plot initialization")
    ...
    def add_axes(self, date):
        ...
        self.ax.text(0.5, 0.95, self.date_string(date), ha="center", va="center", 
                     transform=self.ax.transAxes, fontsize="small")
        self.ax.text(-0.02, 0.99, "Coord: MLT", ha="center", va="top", 
                     transform=self.ax.transAxes, fontsize="small", rotation=90)
        return self.ax
    ...
    def date_string(self, date, label_style="web"):
        ...
        tfmt = "%H:%M"
        date_str = "{:{dd} {tt}} UT".format(date, dd=dfmt, tt=tfmt)
        return date_str
    ...
    def plot_fov(self, df, date, rad, gflg_mask="gflg_ribiero", add_colorbar=True):
        ...
        ax.overlay_fov(rad)
        ax.overlay_radar_data(rad, df, self.proj, self.geodetic, gflg_mask=gflg_mask, 
    ...
    def save(self, filepath):
        print(f"Save FoV figure to : {filepath}")
