...
class FetchData(object):
    ...
    def _parse_data(self, data, s_params, v_params, by, scan_prop):
        ...
        """
        _b, _s = [], []
        if self.verbose: print("Started converting to beam data %02d."%len(data))
        for d in data:
            ...
            if time >= self.date_range[0] and time <= self.date_range[1]:
                ...
                _b.append(bm)
        if self.verbose: print("Converted to beam data.")
        if by == "scan":
            if self.verbose: print("Started converting to scan data.")
            ...
            _s.append(sc)
            if self.verbose: print("Converted to scan data.")
        return _b, _s
    
    def convert_to_pandas(self, beams, s_params=["bmnum", "noise.sky", "tfreq", "scan", "nrang", "time", "channel", "rsep", "frang"],
    ...
    def pandas_to_scans(self, df, smode, s_params=["bmnum", "noise.sky", "tfreq", "scan", "nrang", "time", "channel"],
            v_params=["v", "w_l", "gflg", "p_l", "slist", "v_e"]):
        """
    ...
    def fetch_data(self, s_params=["bmnum", "noise.sky", "tfreq", "scan", "nrang", "intt.sc", "intt.us",\
        ...
        for f in self.files:
            with bz2.open(f) as fp:
                fs = fp.read()
            if self.verbose: print(f"Read file - {f}")
